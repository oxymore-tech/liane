...
public class TripTrackerTest : BaseIntegrationTest
{
  ...
  public async Task ShouldFinishTrip()
  {
    ...
    var tracker = await tripTrackerService.Start(liane, () => { finished = true; });
    ...
    {
      if (finished) break;
      await tripTrackerService.PushPing(liane, new UserPing(p.user, p.timestamp, TimeSpan.Zero, p.coordinate));
    }
  ...
  public async Task MemberShouldNotFinishTrip()
  {
    ...
    var passenger = "63f7a5c90f65806b1adb3081";
    var (liane, geojsonPings) = PrepareTestData(userId);
    var tracker = await tripTrackerService.Start(liane);
    var pings = geojsonPings.Features.Select(f =>
    {
      var point = (f.Geometry as Point)!;
      var time = f.Properties["timestamp"].ToString()!;
    ...
    await tripTrackerService.PushPing(liane, new UserPing(userId, pings.Last().timestamp, TimeSpan.Zero, southWest));
  ...
  public async Task ShouldNotFinishTrip()
  {
    ...
    var lianeDb = BsonSerializer.Deserialize<TripDb>(bson);
    ...
    await Db.GetCollection<TripDb>().InsertOneAsync(lianeDb);
    ...
    {
      if (finished) break;
      await tripTrackerService.PushPing(liane, p);
    }
  ...
  private async Task<(TripTracker, ImmutableDictionary<Ref<User>, Ref<User>>)> SetupTrackerAt(string file, string? at = null)
  {
    ...
    {
      await tripTrackerService.PushPing(tracker.Trip, p);
    }
  ...
  private ITripTrackerService tripTrackerService = null!;

  protected override void Setup(IMongoDatabase db)
  {
    Db = db;
    tripService = ServiceProvider.GetRequiredService<ITripService>();
    tripTrackerService = ServiceProvider.GetRequiredService<ITripTrackerService>();
  }
}