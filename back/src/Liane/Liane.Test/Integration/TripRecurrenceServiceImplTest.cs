...
public sealed class TripRecurrenceServiceImplTest : BaseIntegrationTest
{
  private TripServiceImpl tripService = null!;
  private MockCurrentContext currentContext = null!;
  private ITripRecurrenceService recurrenceService = null!;
  ...
  protected override void Setup(IMongoDatabase db)
  {
    ...
    recurrenceService = ServiceProvider.GetRequiredService<ITripRecurrenceService>();
  }
  ...
  public async Task ShouldCreateRecurrentLiane()
  {
    ...
    var created = await tripService.Create(
      new TripRequest(departureTime, departureTime.AddHours(3), 3, LabeledPositions.PointisInard, LabeledPositions.Tournefeuille, recurrence), bertrand.Id);
    Assert.NotNull(created.Recurrence);

    var lianes = await tripService.List(new TripFilter { ForCurrentUser = true }, new Pagination());
  ...
  public async Task ShouldRepeatRecurrentLiane()
  {
    ...
    var created = await tripService.Create(new TripRequest(departureTime, null, 3, LabeledPositions.PointisInard, LabeledPositions.Tournefeuille, recurrence),
      bertrand.Id);
    Assert.NotNull(created.Recurrence);
    ...
    var updatedLianes = await tripService.List(new TripFilter { ForCurrentUser = true }, new Pagination());
  ...
  public async Task ShouldUpdateRecurrentTrips()
  {
    ...
    lianes = await tripService.List(new TripFilter() { ForCurrentUser = true }, new Pagination());
  ...
  public async Task ShouldUpdateRecurrentTripsWithReturn()
  {
    ...
    var recurrence = DayOfWeekFlag.Create(now.AddDays(4).DayOfWeek, now.AddDays(5).DayOfWeek, now.DayOfWeek);
    var created = await tripService.Create(
      new TripRequest(departureTime, departureTime.AddHours(5), 3, LabeledPositions.PointisInard, LabeledPositions.Tournefeuille, recurrence), bertrand.Id);
  ...
  public async Task ShouldRescheduleRecurrentLiane()
  {
    ...
    var created = await tripService.Create(
      new TripRequest(departureTime, returnTime, 3, LabeledPositions.PointisInard, LabeledPositions.Tournefeuille, recurrence),
      bertrand.Id);
    Assert.NotNull(created.Recurrence);
