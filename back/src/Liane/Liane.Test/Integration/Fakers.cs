...
public class Fakers
{
  ...
  public static readonly Faker<TripRequest> TripRequestFaker = new Faker<TripRequest>()
    .CustomInstantiator(f =>
    {
      var rallyingPoints = f.PickRandom(LabeledPositions.RallyingPoints, 2).ToArray();
      return new TripRequest(f.Date.Soon(15), null, 2, rallyingPoints[0], rallyingPoints[1]);
    });
}