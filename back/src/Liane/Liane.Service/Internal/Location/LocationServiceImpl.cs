...
namespace Liane.Service.Internal.Location
{
    public sealed class LocationServiceImpl : ILocationService
    {
        ...
        public async Task LogLocation(ImmutableList<UserLocation> userLocations)
        {
            if (userLocations.Count >= MinLocTrip)
            {
                    ...
                    .Where(l => l.Count >= MinLocTrip))
                {
                    ...
                    rallyingPointsTrips.Add((await CreateRallyingPoints(trip), trip[0].Timestamp));
                }

                await realTripService.Save(trips.ToImmutable());
                await lianeTripService.Create(rallyingPointsTrips.ToImmutable());

                logger.LogInformation("Created {count} trips, expected 1", trips.Count);
                logger.LogInformation("Created {count} lianes, expected 1", rallyingPointsTrips.Count);
            }
        }
        ...
        private async Task<ImmutableHashSet<RallyingPoint2>> CreateRallyingPoints(ImmutableList<UserLocation> trip)
        {
            ...
            {
                var result = await rallyingPointService.GetFirstClosest(new LatLng(l.Latitude, l.Longitude), MinDistRallyingPoint);
