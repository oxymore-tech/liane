...
import { userSendSms } from '@api/client';
...
const SignUpScreen = ({navigation}: any) => {
  ...
  const actionsOnPress = useCallback(async () => {
    await userSendSms(phoneNumber);
    navigation.navigate("SignUpSms", {phoneNumber: phoneNumber});
  }, [phoneNumber]);
  ...
  return (
    <KeyboardAvoidingView>
      <ImageBackground source={image} style={tailwind("h-full")} resizeMode="cover">
        ...
        <View>
          <AppText
            style={tailwind("text-center text-lg text-gray-600")}>Veuillez entrer votre numéro de téléphone</AppText>
          <AppTextInput
