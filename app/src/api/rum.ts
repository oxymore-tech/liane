...
// CLIENT_TOKEN and APP_ID registered as secrets
// ENV_NAME as any other envi. variable
const { CLIENT_TOKEN, APP_ID, ENV_NAME } = process.env;

// Register the RUM
export async function registerRum() {
  if (CLIENT_TOKEN && APP_ID && ENV_NAME) {
    const config = new DdSdkReactNativeConfiguration(CLIENT_TOKEN, ENV_NAME, APP_ID, true, true, true);
