...
function start {
  create_osm_network
  docker compose -f "${LIANE_HOME}/deploy/osm.yml" -p "osm" up -d --remove-orphans
  docker compose -f "${LIANE_HOME}/deploy/website.yml" -p "liane-website" up -d --remove-orphans
